...
def run_game_show(dict_path: str) -> list[bool]:
        ...
        Parameters:
        dict_path (str):
        The path to the dictionary containing the possible words in this
        DistleGame
        Returns:
        list[bool]:
        ...
        rand_inds = [rng.randint(0, word_count-1) for _ in
        range(GAMESHOW_ROUNDS)]
        sim_results: list[bool] = Parallel(n_jobs=N_CORES,
...
class DistleTests(unittest.TestCase):
        ...
        incomplete set that you are expected to add to to adequately test your
        submission!
        - Your correctness score on the assignment will be assessed by a more
        complete,
        grading set of unit tests.
        - A portion of your style grade will also come from proper type hints; remember
        to
        ...
        def report_results(self, win_list: list[bool]) -> float:
            ...
            Parameters:
            win_list (list[bool]):
            The list of booleans indicating whether or not the player won each
            game.
            Returns:
            float:
            ...
            print("[!] " + self._testMethodName + " Tests: " + str(wins) + " / " +
            str(GAMESHOW_ROUNDS))
